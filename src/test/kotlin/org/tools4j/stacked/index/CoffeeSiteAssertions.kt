...
class CoffeeSiteAssertions(val indexedSiteId: String) {
    ...
    fun assertIsQuestion2(question: Question) {
        ...
        assertThat(question.htmlContent).isEqualTo("<p>Being newly created brewing coffee we have zero feeds appearing in our main chat right now. What blogs, news sites, or other important coffee related things should appear in our main chat room's feed? Post your suggestions/submissions.  </p>\n")
        assertThat(question.userUid).isEqualTo("u$indexedSiteId.2")
    ...
    fun assertHasComment1(comments: List<Comment>) {
        ...
        assertThat(comment.textContent).isEqualTo("*Wave* hello fellow rpg.se user.")
    ...
    fun assertHasComment2(comments: List<Comment>) {
        ...
        assertThat(comment.postUid).isEqualTo("p$indexedSiteId.3")
        assertThat(comment.score).isEqualTo("0")
        assertThat(comment.textContent).isEqualTo("@JoshuaAslanSmith: Cheers. :)")
    ...
    fun assertHasComment3(comments: List<Comment>) {
        ...
        assertThat(comment.textContent).isEqualTo("I've gone ahead and added a feed for this site (meta.coffee) already, since meta's a very important part of the community.")
    ...
    fun assertHasComment4(comments: List<Comment>) {
        ...
        assertThat(comment.score).isEqualTo("0")
        assertThat(comment.textContent).isEqualTo("oh, these are terms for _that_? never heard either… maybe allow both and recommend elaboration in the text?")
    ...
    fun assertHasComment5(comments: List<Comment>) {
        ...
        assertThat(comment.postUid).isEqualTo("p$indexedSiteId.2")
        assertThat(comment.score).isEqualTo("0")
        assertThat(comment.textContent).isEqualTo("@Doorknob add that as an answer so I can upvote (for the meta rep as much as that mattters lol)\\")
    ...
    fun assertHasComment8(comments: List<Comment>) {
        ...
        assertThat(comment.textContent).isEqualTo("@hoc_age Tagging terminology needs to be nailed down in the tag wiki for the site so for our purposes Yes, it does need to be agreed upon and defined. Meta is the place to talk about and discuss issues on the main site.")
    ...
    fun assertHasComment9(comments: List<Comment>) {
        ...
        assertThat(comment.postUid).isEqualTo("p$indexedSiteId.1")
        assertThat(comment.score).isEqualTo("0")
        assertThat(comment.textContent).isEqualTo("@JoshuaAslanSmith - agreed about the tag terminology, and I think  you've convinced me that the best place to \"define\" our *lingua franca* is the tag wiki. My comment on [this answer](http://meta.coffee.stackexchange.com/a/24/262) summarizes my feelings about the (more-)general problem.")
